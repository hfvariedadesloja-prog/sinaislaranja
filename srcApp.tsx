...
export default function App() {
  ...
  const [logs, setLogs] = useState<string[]>([
    ...
    `[${new Date().toLocaleTimeString()}] 📡 Aguardando payloads via Edge Functions.`
  ]);

  // --- PROTOCOLO DE LOGIN (FORT KNOX) ---
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // 💡 Defina a sua senha operacional aqui!
  ...
  const fetchSignals = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('sinais') // Certifique-se de que a sua tabela no Supabase se chama 'sinais'
        .select('*')
  ...
  useEffect(() => {
    if (isAuthenticated) {
      fetchSignals();
      // Polling automático a cada 10 segundos para novos sinais
  ...
  // ==========================================
  // INTERFACE 1: GATILHO DE LOGIN (RESTREITO)
  // ==========================================
  if (!isAuthenticated) {
    return (
    ...
    <div className="min-h-screen bg-[#0B0E11] text-gray-200 font-mono flex flex-col">
      ...
      <main className="flex-1 p-6 space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-[#151A1E] border border-gray-800 rounded p-4 flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-400 uppercase tracking-wider">Taxa de Acerto Média</p>
          ...
          <div className="bg-[#151A1E] border border-gray-800 rounded p-4 flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-400 uppercase tracking-wider">Alertas Ativos</p>
        ...
        {/* TABELA DE SINAIS INSTITUCIONAIS */}
            ...
            <table className="w-full text-left text-sm border-collapse">
              <thead>
                <tr className="bg-[#0B0E11] text-gray-400 text-xs border-b border-gray-800 uppercase tracking-wider">
                  ...
                  <th className="px-6 py-3">ADX</th>
                  <th className="px-6 py-3">RVOL</th>
                ...
                {signals.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-8 text-center text-gray-500 text-xs">
                      Nenhum payload detetado na tabela. Dispare um sinal do Python para testar!
                    </td>
                  </tr>
                ) : (
                  signals.map((sig, i) => (
                    <tr key={sig.id || i} className="hover:bg-gray-800/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2 py-0.5 rounded text-xs font-bold ${
                          sig.lado === 'LONG' ? 'bg-[#0ECB81]/10 text-[#0ECB81]' : 'bg-[#F6465D]/10 text-[#F6465D]'
                        }`}>
                          {sig.lado}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-white font-mono">${Number(sig.preco_entrada).toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                      <td className="px-6 py-4 text-gray-300">{sig.adx}</td>
                      <td className="px-6 py-4 text-gray-300">{sig.rvol}x</td>
                      <td className="px-6 py-4">
